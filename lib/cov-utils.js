...
function setupCoverage(projectRoot, coverageName, config, options={'saveReport': true, 'saveRawData': true }) {
  ...
  process.on("exit", () => {
    ...
    if (options['saveRawData']) {
      saveRawCoverage(projectRoot, coverageMap, coverageName, config)
    }

    if (options['saveReport']) {
      reportCoverage(projectRoot, coverageMap, config);
    }
  });
}
...
function saveRawCoverage(projectRoot, coverageMap, coverageName, config = {}) {
...
function mergeCoverageFromRaw(projectRoot, config = {}) {
...
function reportCoverage(projectRoot, coverageMap, config = {}) {
  ...
  options["dir"] = getReportDir(config)
